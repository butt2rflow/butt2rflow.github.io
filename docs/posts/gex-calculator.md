...
2. **CBOE CSV 형식 의존** — 파이썬 코드는 CBOE CSV의 컬럼 순서를 하드코딩합니다. CBOE가 CSV 형식을 변경하면 `df.columns` 배열을 수정해야 합니다.

3. **CBOE Gamma는 BSM 모델 기반** — 이론적 감마입니다. 실제 MM이 사용하는 감마와 다를 수 있습니다.

4. **OI의 방향을 모릅니다** — 누가 매수이고 매도인지 공개 데이터로는 알 수 없습니다. "콜 = MM 매수, 풋 = MM 매도" 가정에 의존합니다.

5. **0DTE가 지배적** — ATM 0DTE 감마가 극대이므로, GEX가 당일 만기 하나에 좌우됩니다. 장중에 0DTE OI가 변하면 GEX도 크게 변합니다.

6. **절대적 매매 신호가 아닙니다** — GEX는 "MM의 예상 포지션"을 추정하는 방향성 지표입니다. 단독으로 매매 결정에 사용하지 마세요.
