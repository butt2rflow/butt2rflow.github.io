...
> 📚 **이 글의 위치**: COR/SKEW *추적·해석 도구* (이 글). 지수들의 *분류 체계와 개념적 배경*은 [시장 심리 변동성 지수](implied-correlation.md). 둘이 짝.
...
**내재상관관계**는 이 "동조화 수준"을 0~100%로 수치화한 것입니다. S&P500 지수 옵션의 내재변동성(IV: Implied Volatility — 옵션 가격에 녹아든 미래 변동성 기대치)과 상위 50개 종목의 개별 옵션 IV 사이의 관계로 계산합니다.
...
![내재상관관계 — 분산 vs 동조화](./diagrams/diag_vol_correlation_concept.png)
...
![Cboe COR 지수 2축 구조](./diagrams/diag_vol_cor_axes.png)
...
| SKEW 수준 | 의미 |
|:---------|:-----|
| ~145 이하 | 평상시 (post-2020 중앙값 145 근처) |
| 150~158 | 경계 — 기관의 풋 매수 증가 (p75 영역) |
| 158+ | 높은 꼬리 위험 인식 (p95 위) |

!!! note "SKEW의 역사적 변화 — 임계 재캘리브레이션 (2026-05)"
    2020년 이전에는 SKEW 평균이 ~115였고 140 이상은 매우 드물었습니다. 2020년 이후 구조적으로 상향 이동하여 **현재 median ≈ 145, p75 ≈ 150, p95 ≈ 157**. 옛 임계 140/150은 "기본 상태"가 돼버려 의미를 잃었기에, 라이브 대시보드는 **150 경계 / 158 스트레스**로 재캘리브레이션(p75/p95)했습니다. 위 표는 그 기준입니다.
...
![변동성 대시보드 — Term Structure + Delta Skew + SKEW 통합 차트](../assets/diagrams/vol_dashboard.png)

상단: COR1M~COR1Y Term Structure (간격 좁아지면 경계, 빨간 영역 = 역전). 중단: COR10D~COR90D Delta Skew (COR90D가 주황 점선 50 위 = 경계, 빨간 점선 60 위 = 스트레스). 하단: SKEW 지수 (150 이상 경계, 158 이상 스트레스).
...
| 지표 | 정상 | 경계 | 스트레스 |
|:----|:-----|:-----|:--------|
| **COR90D 수준** | ~45 이하 | 50~60 | **60 이상** |
| **COR90D - COR10D 차이** | 25+ (넓게 분산) | 좁아지는 중 | 수렴 (모든 행사가가 1.0 근접) |

COR90D(깊은 OTM call 영역)가 **60 이상**으로 치솟으면, 시장 전체가 한 방향으로 동조화되고 있다는 뜻입니다. 차이가 좁아지면 모든 행사가에서 상관관계가 1.0으로 수렴 — 계란을 여러 바구니에 나눠 담았는데 바구니가 모두 같은 선반 위에 있었던 것입니다. 선반이 넘어지면 전부 깨집니다. 분산 투자가 작동하지 않는 상태입니다.

!!! note "임계 재캘리브레이션 (2026-05)"
    옛 35/50 임계는 pre-2020 분포 기준이었습니다. post-2020 regime에서는 COR90D **median ≈ 50**, p75 ≈ 62, p95 ≈ 79 — 50이 "기본 상태"가 되었기에 50/60(p75/p95)로 재캘리브레이션해 상위 25% / 상위 5%를 가르도록 했습니다.

**예시 — 경계 (2025-10-16):**

```
COR90D = 54.60 ← 50~60 사이, 경계 (단일 팩터 regime 진입 초입)
COR10D = 10.57
```
...
|:-------------|:---------|:-----|:---------|:----|
| 넓음 (10+) | ~45 이하 | <150 | **안정** | 리밸런싱 정상 진행 |
| 좁아지는 중 (<5) | 50~60 | 150~158 | **경계** | 리밸런싱 타이밍 주의 |
| 역전 | **60 이상** | **158 이상** | **스트레스** | 급변동 가능, 원칙 고수 |
| 역전 후 회복 | 하락 중 | 하락 | **회복** | 리밸런싱 기회 |

핵심: **세 신호가 동시에 악화되면 구조적 스트레스**입니다. 하나만 악화되면 노이즈일 수 있습니다.

![세 신호 종합 판단](./diagrams/diag_vol_three_signals.png)
...
- [ ] COR90D 수준: ~45 이하 (정상) / 50~60 (경계) / 60 이상 (스트레스)
- [ ] SKEW: 정상 / 경계 / 높음
- [ ] 세 신호 동시 악화 여부
...
<details><summary>Python: 변동성 대시보드 차트 코드</summary>
...
<details><summary>Python: 스프레드 자동 계산 + 경고 코드</summary>
...
print("  ← 깊은 역전!" if spread < -5 else "  ← 역전·경계" if spread < 0 else "  ← 정상")

cor90d = latest_skew['COR90D']
print(f"COR90D: {cor90d:.1f}", end="")
print("  ← 스트레스" if cor90d > 60 else "  ← 경계" if cor90d > 50 else "  ← 정상")

skew_val = latest_skew['SKEW']
print(f"SKEW: {skew_val:.1f}", end="")
print("  ← 스트레스" if skew_val > 158 else "  ← 경계" if skew_val > 150 else "  ← 정상")
```
