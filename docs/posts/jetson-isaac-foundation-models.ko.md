...
> **현장 노트 · '로봇의 뇌를 엣지에' 2부작 중 2부.** [1부](jetson-ros2-setup.md)에선 맨 Jetson 보드에 JetPack을 깔고 **ROS 2**까지 올렸습니다. 이번 편은 그 위에 **Isaac ROS**와 **파운데이션 모델 파이프라인**을 얹었어요. "학습 없이 새 부품을 잡는다"는 그 약속이, 우리가 가진 **작은 엣지 보드에서도 정말 도는지** 직접 확인해 봤습니다.
...
1부의 큰 그림이 여기서도 그대로입니다. 무거운 학습은 데스크톱이나 클라우드에서, 추론은 엣지에서. 이번 편이 더한 건, 그 "추론" 칸에 파운데이션 모델을 실제로 앉혀보고, 가진 보드에서 도는 걸 확인한 겁니다.
