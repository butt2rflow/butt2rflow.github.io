...
그래서 실무에서는 **Half-Kelly** 또는 **Quarter-Kelly**를 씁니다 ([원칙편 시리즈](../series/index.md) Part 4 "우위 없는 게임" 유료편 참조). 일일 대시보드에는 절충점 **¾ Kelly**도 토글로 추가되어 있습니다 — VIX 19+ 영역에서 Half(과보수)와 Full(과공격) 사이의 의미있는 중간점.
...
- **Half (½)**: 학계 표준. **CAGR 변동성이 최소화되는 지점** — 즉 "장기 성장률은 약간 떨어지지만 경로의 흔들림이 가장 작은" 절충점. [원칙편](../series/index.md) Part 4 "우위 없는 게임"의 보수성 권고와 일치.
...
2. **시간이 우리의 우위** — 리테일은 환매 압력 없이 기다릴 수 있는 *구조적 우위*가 있다

이 둘을 합치면 함의가 명확해집니다: **향후 위험 프리미엄이 가장 확대되는 순간 = 위기 한복판**. 그 순간 자본을 투입할 메커니즘이 없으면, 시간 우위를 *가지고 있다*고 말하면서 *가장 비싼 순간에 안 쓰는* 모순.
...
| **공격 자본** | 공격 — *공격을 위한 연료* | VIX 극단 복합 트리거에 1/3씩 단계 투입. 시간 우위 행사. |
...
> **결론**: 비율 자체는 절대 흔들지 말고, *대신* 공격 자본을 메인 프레임워크의 짝으로 두세요. 프레임워크는 *생존*을 책임지고, 공격 자본은 *시간 우위를 활용* 합니다. 이 둘이 합쳐져야 시리즈의 **"변동성을 연료로"** 가 진정 완성됩니다.
...
| 상황 | 추천 Split | 근거 |
|:---|:---:|:---|
| 적립기 (장기 horizon, 적립 초반) | **95/5** | 시간 우위로 복리를 극대화. T3 같은 진짜 투매(panic selling)에만 보조 화력. |
