...
### 심화: 위기 한복판에서 Kelly fraction을 올려도 되는가?

자주 나오는 질문 — 폭락 와중에 Half에서 Full로 바꾸는 게 무모한가?

**수학적으로는 옹호 가능**합니다. 가격이 빠질수록 forward μ−r이 확대된다는 가정이 합리적이기 때문 (평시 5% → 위기 8~10%). 예: VIX 60 + μ−r=10%로 Full Kelly = 28%, 같은 VIX의 Half Kelly @ 5% = 7%. 차이가 4배여도 80%+ 현금 상태에서의 *절대 위험*은 여전히 작습니다.

**그럼에도 fraction 자체를 흔드는 건 권하지 않습니다:**

- **떨어지는 칼날** — VIX > 60은 바닥처럼 보이지만 2008년에는 1년 사이 두 번. "충분히 떨어졌나"는 사후에만 답할 수 있음.
- **추정 오차의 역설** — σ가 가장 불확실한 시점에 μ 추정도 가장 불확실. 그때 fraction을 올리는 건 *가장 흔들리는 곳에서 베팅을 키우는* 행동.
- **행동심리적 위험** — 공포 극단에서 토글을 만지는 건 결국 감정적 결정. Kelly fraction 고정의 핵심 이유(감정 배제)가 무너짐.
- **트리거의 backfit 위험** — "VIX > 50 5일 지속" 같은 규칙은 과거 데이터로 잘 작동하는 것만 골라 만든 것이라 미래 보장 없음.

### Tactical bucket은 framework의 *필수 짝*
...
이 둘을 합치면 함의가 명확해집니다: **forward risk premium이 가장 확대되는 순간 = 위기 한복판**. 그 순간 자본을 deploy할 메커니즘이 없으면, 시간 에지를 *가지고 있다*고 말하면서 *가장 비싼 순간에 안 쓰는* 모순.

**Defensive framework만 있을 때의 빈자리:**

- Kelly × VIX 자동 디레버리지로 100% → 2% 갈 수 있음 ✓ (cascade 흡수)
- VIX가 식으면 자동 재진입 → 그러나 *회복기 가격*에 사게 됨, *바닥 가격이 아니라*
- 결과: vol drag는 피하지만 진짜 buy-low는 못 함. [Shannon's Demon](../series/s1-shannons-demon.md)이 끝맺은 "리밸런싱 = buy low + sell high" 중 sell high 쪽만 동작

따라서 **현금을 두 종류로 분리**하는 게 시리즈 철학과 맞물리는 완결된 구조:

| 자본 분리 | 역할 | 메커니즘 |
|:---|:---|:---|
| **메인 (~80%)** | Defensive — *생존을 위한 연료* | Kelly × VIX + 위험 신호. 자동 디레버리지·재진입. |
| **Tactical (~20%)** | Offensive — *공격을 위한 연료* | VIX 극단 복합 트리거에 laddered deploy. 시간 에지 행사. |

### Tactical bucket 운영 원칙

- **사이즈**: 전체 포트폴리오의 10~20%. 너무 크면 메인 framework의 안정성을 흔들고, 너무 작으면 deploy 효과가 미미.
- **복합 트리거** (단일 지표 X — 동시 충족 조건):
    - VIX > 50 *지속* (단발 spike 제외, 5일 이상)
    - COR90D > 55 + SKEW > 150 동시
    - 30일 누적 SPX 하락 −20% 이상
- **Laddered deploy** — 한 번에 다 쓰지 말고 분할:
    - 1차 트리거 발동 시 tactical의 1/3 deploy
    - VIX 추가 +10p 진행 시 또 1/3
    - VIX 추가 +10p 또는 결정적 capitulation 시 마지막 1/3
- **회복기에 천천히 재충전** — 평시로 돌아오면 메인 framework의 익스포져 감소분 또는 차익으로 tactical bucket 재충전. 한 번 deploy 후 영원히 비워두지 말 것.
- **사전 정의된 규칙으로만 발동** — "느낌" 아닌 *사전에 적어둔 조건*에서만. 조건 미달이면 가만히. (감정적 진입 차단)

> **결론**: Fraction 자체는 절대 흔들지 말고, *대신* tactical bucket을 메인 framework의 짝으로 두세요. Framework는 *생존*을 책임지고, tactical bucket은 *시간 에지를 monetize* 합니다. 이 둘이 합쳐져야 시리즈의 **"변동성을 연료로"** 가 진정 완성됩니다.
...
|:---|:---|:---|:---|:---|
| **COR/SKEW** | Term Structure spread · COR90D · SKEW | 모두 정상 | 하나 caution | 하나 이상 danger |
...
## 9. 실전 시나리오 — 첫 갭은 못 피해도 cascade는 깎는다

이 framework의 가치는 **폭락의 첫 갭**이 아니라 **그 뒤 누적**에서 나옵니다. 첫 -3%, -5% 갭은 인사이더나 우연을 빼면 누구도 못 피하는 영역이라 framework의 한계로 볼 이유가 없습니다. 진짜 차이는 *그 다음 며칠~몇 주*에서 발생합니다.

| 단계 | 시장 움직임 | 자동 반응 |
|:---|:---|:---|
| **Day 1 갭** | 폭락 -5% | 풀 포지션 그대로 맞음 (반응형 framework의 진입 비용) |
| **Day 2 ~ N** | VIX 폭증 → σ² 4배 → Kelly 베이스 1/4 이하 + 위험 신호 빨강 × 0.42 | 익스포져가 *이미* 작아진 상태에서 후속 폭락 흡수 |
...
VIX 14 → 82로 6배 뛰면서 권장 비중은 100% → 2%. 이 한 달 사이 시장이 추가로 약 −25% 더 빠졌는데, 그 cascade는 *이미 줄어든* 익스포져로 맞기 때문에 정적 풀투자 대비 누적 손실이 크게 줄어듭니다.

**2008년 9월 리먼 사태:**

리먼 파산 발표(9/15) 직후 첫 주 −10%는 그대로. 하지만 그 뒤 한 달간 추가 −25% 하락은, framework가 자동으로 디레버리지된 상태에서 맞기 때문에 누적 손실이 정적 포지션의 절반 수준 (개념적 예시).

> **요약**: framework는 *예측 도구*가 아니라 *cascade buffer*. 첫 갭은 진입 비용으로 받아들이고, 그 뒤 누적 손실을 줄이는 게 핵심 가치.
...
1. **μ − r = 5% 가정의 위험성**: 미래 equity premium이 4% 또는 3%라면 권장 비중이 과대평가됩니다. 본인이 더 보수적이라고 생각하면 Quarter-Kelly로 자동 보정 가능.
