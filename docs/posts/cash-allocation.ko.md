...
| 균형기 (적립 후반, 자산 형성기) | **90/10** | 평시 드래그 최소화하면서 VIX>60 진입은 의미 있게 유지. |
| 보수기 (은퇴 직전, 인출 단계, 행동심리 약점) | **80/20** | 시퀀스 리스크 방어. 폭락 시 정신적 안정과 매수 화력 동시 확보. |

> **솔직히**: 80/20은 *행동심리적 보험료* 측면이 큽니다. 수학적으로는 Kelly 자체가 이미 half-Kelly + multiplier로 보수적이고, σ=VIX/100 으로 변동성에 자동 반응하므로 거기에 20% 현금을 또 빼면 트리플 안전마진입니다. 본인이 폭락 한복판에서도 토글을 함부로 안 만질 자신이 있다면 90/10 또는 95/5가 합리적입니다. 자신 없으면 80/20이 안전합니다.
...
2. **위험 민감도 토글** (느슨 / 기본 / 빡빡) — 신호 발동 시 얼마나 깎을지
3. **μ−r 토글** (5% / 7% / 9%) — 본인의 주식 프리미엄 가정. 5%=보수(VRP 보정), 7%=역사적, 9%=공격적
4. **결정 테이블** — 베이스값 + 각 그룹 배율 + 최종 주식/현금 % 한눈에
5. **Kelly 곡선 차트** — VIX → 비중의 관계를 시각화 (5% μ−r 기준), 현재 위치 마커
...
1. **μ − r 가정의 변동성**: 미래 주식 위험 프리미엄을 정확히 알 수 없습니다 — 학계 추정치 범위는 3%~9% 매우 넓음. 일일 대시보드의 **μ−r 토글(5% / 7% / 9%)** 로 본인 가정을 바꿔볼 수 있습니다. 보수적 5%(default)는 VRP 보정 포함, 7%는 SPX 역사적 평균, 9%는 post-1990 bullish view.
2. **VRP 편향**: VIX는 평균적으로 실현변동성보다 3~5포인트 높음 (변동성 위험 프리미엄, VRP). 즉 5% μ−r에서 σ=VIX로 푼 Kelly 베이스는 *상당히 보수적*으로 나옴 — μ−r=7~9%가 사실상 realized-vol Kelly에 더 가까운 값.
