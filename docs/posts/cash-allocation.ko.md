...
### 공격 자본 (Tactical bucket)은 framework의 *필수 짝*
...
| **공격 자본 (~20%)** | Offensive — *공격을 위한 연료* | VIX 극단 복합 트리거에 1/3씩 단계 투입. 시간 에지 행사. |
...
- **회복기에 천천히 재충전** — 평시로 돌아오면 메인 framework의 익스포져 감소분 또는 차익으로 공격 자본을 재충전. 한 번 투입 후 영원히 비워두지 말 것.
- **사전 정의된 규칙으로만 발동** — "느낌" 아닌 *사전에 적어둔 조건*에서만. 조건 미달이면 가만히. (감정적 진입 차단)

> **결론**: Fraction 자체는 절대 흔들지 말고, *대신* 공격 자본을 메인 framework의 짝으로 두세요. Framework는 *생존*을 책임지고, 공격 자본은 *시간 에지를 monetize* 합니다. 이 둘이 합쳐져야 시리즈의 **"변동성을 연료로"** 가 진정 완성됩니다.
