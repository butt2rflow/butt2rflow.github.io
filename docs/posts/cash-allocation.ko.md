...
| 접근 | 방식 | 장단점 |
|:---|:---|:---|
| **A. 그냥 두기 (권장)** | Framework가 이미 자동으로 현금 비중을 늘려뒀음. Half + 자동 디레버리지로 cascade 흡수, 회복기에 자동 재진입. | 가장 단순. 원칙편 No-Edge 철학과 일관. |
| **B. 별도 *tactical bucket*** | 메인 포트폴리오는 Half 유지. "위기 매수 자금" (전체의 10~20%)을 별도로 두고 VIX > 50 등 트리거에 deploy. | Kelly fraction 자체는 안 건드림. 위기 매수 본능과 framework 안정성을 분리. |

> **결론**: A가 기본. "위기에 사고 싶다"는 본능을 살리고 싶으면 B를 부수적으로. *fraction 자체를 동적으로 흔드는 건 framework의 핵심 설계(감정 배제, 일관된 비율)를 깨는 행동이라 권하지 않습니다.*
