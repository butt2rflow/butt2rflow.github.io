...
그래서 실무에서는 **Half-Kelly** 또는 **Quarter-Kelly**를 씁니다 ([원칙편 시리즈](../series/index.md) Part 4 "엣지 없는 게임" 유료편 참조). 라이브 대시보드에는 절충점 **¾ Kelly**도 토글로 추가되어 있습니다 — VIX 19+ 영역에서 Half(과보수)와 Full(과공격) 사이의 의미있는 중간점.
...
라이브 대시보드의 마스터 바에서 **80/20 · 90/10 · 95/5** 중 선택할 수 있습니다. 결론부터: **트레이드오프는 "평시 현금 드래그 vs 위기 시 매수 화력"** 입니다.
...
1. **μ − r 가정의 변동성**: 미래 주식 위험 프리미엄을 정확히 알 수 없습니다 — 학계 추정치 범위는 3%~9% 매우 넓음. 라이브 대시보드의 **μ−r 토글(5% / 7% / 9%)** 로 본인 가정을 바꿔볼 수 있습니다. 보수적 5%(default)는 VRP 보정 포함, 7%는 SPX 역사적 평균, 9%는 post-1990 bullish view.
