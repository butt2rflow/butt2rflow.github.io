...
|:---|:---:|:---|
| 80/20 | +20% 매수 | VIX 60 + 항복 매수에서 의미 있는 진입 |
...
| 상황 | 추천 Split | 근거 |
|:---|:---:|:---|
| 적립기 (장기 horizon, 적립 초반) | **95/5** | 시간 에지로 복리를 극대화. T3 같은 진짜 항복에만 보조 화력. |
...
| 프로파일 | μ−r 값 | 근거 | VIX 18에서 Half | VIX 22에서 Half | VIX 30에서 Half |
|:---|:---:|:---|---:|---:|---:|
| **보수 (Conservative)** | **5%** | 장기 학계 추정 (Damodaran 등). VIX의 vol-risk-premium 흡수 가정 — 즉 VIX = realized σ + 4pt가 평균이라 σ를 좀 부풀려 푸는 셈. | 74% | 52% | 28% |
| **표준 (Standard) ← 디폴트** | **7%** | 1928년 이후 SPX 역사적 평균 ex-WWII. realized vol 기준 Kelly에 사실상 가장 가까운 값. | 100% (cap) | 72% | 39% |
| **공격 (Aggressive)** | **9%** | post-1990 SPX bullish 가정. VIX의 vol-risk-premium drag를 *완전히 회복 가능한* 비효율로 봄. | 100% (cap) | 93% | 50% |
