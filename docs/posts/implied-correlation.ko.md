...
**VIX (현물 지수)**는 시장의 공포 온도계로, SPX 옵션의 광범위한 외가격(OTM) 행사가(콜+풋)를 사용하여 **향후 30일간의 기대 변동성**을 계산하는 대표적인 시장 지표입니다. 등가격(ATM: At The Money — 현재가 근처) 단일 지점이 아니라, IV Surface의 넓은 영역을 종합적으로 반영합니다. 즉, SPX 지수 옵션 시장 참여자들이 **30일 후 시장의 변동성을 예측하고 암묵적으로 동의한 추정값**입니다.

<details><summary>VIX 계산 방법 (심화)</summary>

VIX는 변동성을 통째로 추적하는 구조인 분산스왑(variance swap) 복제 방식을 사용합니다. 단일 행사가의 IV가 아니라, 다양한 OTM 행사가의 옵션 가격을 가중 합산하여 30일 기대 분산을 계산하고, 그 제곱근을 연율화하여 지수로 표현합니다.
...
S&P500 지수 옵션에는 [변동성 왜곡(Volatility Skew)](skew.md) 현상이 있습니다. 내재상관관계의 Delta Skew 지수들을 한 그래프에 그리면, IV 왜곡의 곡선이 펴져서 **거의 직선에 가까운 모양**으로 표시됩니다. 이 직선의 기울기로 Skew 정도를 한 눈에 비교할 수 있습니다.
...
*이전 글: [Hedging the Wings](hedging-wings.md) | 관련 글: [변동성 Skew](skew.md) | [신용 스프레드를 지표로 읽기](credit-spreads.md) | [MOVE 지수 — 채권시장의 VIX](move-index.md) | [VIX 기간구조](vix-term-structure.md)*
