...
그래서 정리하면: **감마의 영향은 실재하지만(일부는 학술로도 확인), 평균 규모는 작고, 방향(부호) 판단은 더 불확실합니다.** GEX는 파도를 *예측*하는 도구가 아니라, 그 파도가 *증폭될 바다냐 잔잔한 바다냐(레짐)*를 알려주는 지표로 쓰세요. 참고는 하되 맹신은 금물 — 부호는 추정이고, OI 레벨은 전일 밤 스냅샷이라 장중 무효화 되기 시작하고, 업체마다 값도 다르니까요. (감마 너머의 딜러 흐름과 그 *규모 논쟁*은 [감마 너머 — Vanna·Charm](vanna-charm.md)에서 더 깊게.)
