...
그리고 이 글은 *계산법*만이 아니라 **그 값을 어떻게 읽어야 하는지**까지 답해요. GEX는 파도를 맞히는 '신호'가 아니라, 지금 바다가 어떤 상태인지 알려주는 *지도*거든요 — **감마는 잔물결, 파도는 델타**. (자세한 건 마지막 섹션에서.)
...
1. **콜옵션 미결제약정(OI: Open Interest — 아직 청산되지 않은 계약 수)** = 투자자가 매도(SPX/NDX에서는 기관이 보유 주식에 대해 콜옵션을 파는 전략(커버드콜)이 지배적), MM이 매수 → **MM 롱 감마 (+)**
...
![MM 감마 포지션에 따른 시장 영향](./diagrams/diag_gex_mm_gamma.png)
...
→ Max Pain: 만기일에 옵션 매수자 전체가 가장 적게 버는 가격 — 쉽게 말해, 옵션을 산 사람들이 가장 울상인 가격입니다. 현재가(5,976.97)와 가까우면 만기일에 주가가 특정 행사가에 붙어 움직이지 않는 현상("핀닝(pinning)") 가능성.
...
![SPX GEX 프로파일 — ATM 부근 숏 감마 집중 구조](../assets/diagrams/gex_profile.png)
...
![GEX 공식 분해](./diagrams/diag_gex_formula.png)
...
![감마 플립 포인트 개념](./diagrams/diag_gex_flip_point.png)
...
!!! note "Max Pain, 얼마나 믿나"
    솔직히 제한적입니다. 만기일에 (옵션이 있는) **개별 주가**가 큰 OI 행사가에 달라붙는 '핀닝'은 학술로 확인됐지만(Ni·Pearson·Poteshman, 2005), 그건 개별주 얘기예요 — **지수(SPX)가 Max Pain으로 끌려간다**는 강한 증거는 약합니다. 그리고 의미가 있어도 **만기 근처**에서지, 평상시 장중엔 신뢰도가 낮아요. 널리 회자되는 만큼 **과대평가**된 지표이니 *만기일 참고치* 정도로만 쓰세요. (핀닝·딜러 흐름의 실증은 [감마 너머 — Vanna·Charm](vanna-charm.md)에서 더 깊게.)
...
<details><summary>파이썬 전체 코드 (GEX + Max Pain + 차트)</summary>
...
2. **CBOE Gamma는 블랙-숄즈 모형(BSM) 기반** — 이론적 감마입니다. 실제 MM이 사용하는 감마와 다를 수 있습니다.
...
## 감마는 잔물결, 파도는 델타 — 직접 트레이딩하며 배운 것

솔직한 경험담 하나. 저도 한때 GEX를 보며 트레이딩했는데, 몸으로 깨달은 건 이거였어요 — **감마 헷지는 잔물결이고, 강한 파도는 결국 대량 매수·매도, 즉 델타(실제 주문 흐름)에서 온다.**

- **파도 = 델타(수급).** 가격을 진짜 미는 건 대량 매수·매도 — 인덱스 펀드 리밸런싱, 기계적·규칙 기반 전략, 큰손의 방향성 주문. 이게 진짜 몸통이에요.
- **잔물결 = 감마 헷지.** 딜러가 델타 중립을 유지하려 찔끔찔끔 따라 사고파는 것. 평균적으론 작아요 (실제로 CBOE 자료에서도 딜러 순감마는 하루 선물 유동성의 **0.04~0.17%** 수준).

그럼 감마는 무의미하냐? 아니에요. 감마의 진짜 역할은 **파도를 만드는 게 아니라, 델타가 만든 파도를 키우거나 잠재우는 것**입니다. 이때 나오는 말이 **레짐(regime, 지금 시장이 어떤 '모드'냐)**이에요 — **숏 감마**면 딜러가 흐름과 같은 방향으로 헷지해 파도를 **증폭**하고, **롱 감마**면 역방향으로 헷지해 **완충**합니다.

![감마는 잔물결, 델타는 파도 — 숏/롱 감마의 증폭·완충](../assets/diagrams/gex-ripple-wave.svg)

가끔은 잔물결이 큰 방향의 **방아쇠가 되기도** 해요 — 팽팽히 당겨진 수급 위, 불안정한 지점(숏 감마 · 저유동성 · 핵심 행사가 근처)에서요(감마 스퀴즈, 나비효과처럼). 다만 **감마는 방아쇠지 연료가 아닙니다** — 에너지는 델타에 저장돼 있어요.

그래서 정리하면: **감마의 영향은 실재하지만(일부는 학술로도 확인), 평균 규모는 작고, 방향(부호) 판단은 더 불확실합니다.** GEX는 파도를 *예측*하는 도구가 아니라, 그 파도가 *증폭될 바다냐 잔잔한 바다냐(레짐)*를 알려주는 지표로 쓰세요. 참고는 하되 맹신은 금물 — 부호는 추정이고, OI 레벨은 전일 밤 스냅샷이라 장중엔 실제와 어긋나기 시작하고, 업체마다 값도 다르니까요. (감마 너머의 딜러 흐름과 그 *규모 논쟁*은 [감마 너머 — Vanna·Charm](vanna-charm.md)에서 더 깊게.)
...
GEX는 단기 매매 신호가 아닙니다. 장기 투자자에게 GEX의 가치는 **시장의 급변동을 구조적으로 이해하고, 공포에 흔들리지 않는 것**입니다. 지수가 갑자기 -2% 빠졌을 때 "0DTE 감마 헷지 물량이구나"라고 판단할 수 있으면, 패닉에 팔지 않고 리밸런싱 원칙을 지킬 수 있습니다.
...
- [Cboe — SPX 0DTE 옵션의 시장 영향 평가](https://www.cboe.com/insights/posts/volatility-insights-evaluating-the-market-impact-of-spx-0-dte-options) (딜러 순감마 규모)
...
*이전 글: [Almanac Trader — Seasonality 분석](almanac.md) | 다음 글: [0DTE 감마 패턴 — 장중 GEX는 어떻게 변하는가](gex-0dte-patterns.md)*
