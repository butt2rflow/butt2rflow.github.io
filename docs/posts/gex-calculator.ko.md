...
!!! note "Max Pain, 얼마나 믿나"
    솔직히 제한적입니다. 옵션 만기일에 **개별 주가**가 큰 OI 행사가에 달라붙는 '핀닝'은 학술로 확인됐지만(Ni·Pearson·Poteshman, 2005), 그건 개별주 얘기예요 — **지수(SPX)가 Max Pain으로 끌려간다**는 강한 증거는 약합니다. 그리고 의미가 있어도 **만기 근처**에서지, 평상시 장중엔 신뢰도가 낮아요. 널리 회자되는 만큼 **과대평가**된 지표이니 *만기일 참고치* 정도로만 쓰세요. (핀닝·딜러 흐름의 실증은 [감마 너머 — 바나 랠리·핀닝](vanna-charm.md)에서 더 깊게.)
...
그래서 정리하면: **감마의 영향은 실재하지만(일부는 학술로도 확인), 평균 규모는 작고, 방향(부호) 판단은 더 불확실합니다.** GEX는 파도를 *예측*하는 도구가 아니라, 그 파도가 *증폭될 바다냐 잔잔한 바다냐(레짐)*를 알려주는 지표로 쓰세요. 참고는 하되 맹신은 금물 — 부호는 추정이고, OI 레벨은 전일 밤 스냅샷이라 장중엔 실제와 어긋나기 시작하고, 업체마다 값도 다르니까요. (감마 너머의 딜러 흐름과 그 *규모 논쟁*은 [감마 너머 — 바나 랠리·핀닝](vanna-charm.md)에서 더 깊게.)
