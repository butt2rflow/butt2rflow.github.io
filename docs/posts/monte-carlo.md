...
`RAND()` 함수는 호출할 때마다 **0에서 1 사이의 임의의 실수**를 동등한 확률로 돌려주는 함수입니다. 즉, 모든 값이 같은 확률로 나오는 균등분포(Uniform Distribution)를 따릅니다.
...
<details><summary>구글 시트 커스텀 함수 코드</summary>
...
<details><summary>LOGINV() 파라미터 주의사항</summary>

`LOGINV()`의 파라미터는 로그정규분포(주가처럼 0 이하로 떨어지지 않는 데이터에 적합한 분포) 자체의 평균/표준편차가 아니라, **기저 정규분포(ln(X))의 평균과 표준편차**입니다.
...
<details><summary>파이썬 전체 코드</summary>
