...
전부를 그림 하나로 보면 이렇습니다. **같은 자리**에 있는 점 하나가, 좌표계마다 다른 숫자를 갖습니다. 바꾸려면 회전한 다음, 이동하면 돼요.
...
이게 여러 단계로 확장되는 비결입니다. **카메라 → 손목**을 알고 **손목 → 베이스**를 알면, 둘을 차례로 적용해서 **카메라 → 베이스**를 얻어요. 몇 단계를 거치든 상관없습니다. 좌표계가 다섯이면 변환 넷을 줄줄이 잇는 것뿐이죠.
...
실제 파이프라인에서 재밌는 점은, 링크 셋이 저마다 *다른 데서* 온다는 거예요. 하나는 AI, 하나는 고정, 하나는 실시간.
...
2. 두 좌표계를 오가려면 **변환**이 필요하다 — 이동과 회전, 숫자 여섯 개.
3. 적용할 땐 **먼저 회전, 그다음 이동.** 순서가 중요하다.
...
*관련: [스테레오 비전에서 잡기까지 — 로봇은 목표한 사물을 어떻게 발견하는가](stereo-to-grasp.md)*
...
- *이동(translation) · 회전(rotation)* — 위치를 옮기는 것과 방향을 트는 것. 회전 먼저, 그다음 이동
