...
이렇게 매주 선물시장의 참여자들이 보고하는 미결제약정을 각각의 비즈니스 범주로 나눠서 롱 포지션(가격 상승에 베팅한 매수)과 숏 포지션(가격 하락에 베팅한 매도)의 총합계를 보여주는 것을 **COT(Commitments of Traders) 리포트**라고 하며, 매주 **금요일 미 동부시간 3:30 PM**에 발표됩니다.
...
1. **Commercial** — 현물을 직접 사고 파는 거래를 하는 참여자 (헷저 — 위험을 줄이기 위해 반대 방향 거래를 동시에 가지는 참여자)
...
- COT 데이터는 CFTC 웹사이트(cftc.gov)에서 직접 확인하거나, Barchart.com 등의 차트에서 시각적으로 확인 가능 — 또는 이 블로그 [홈 대시보드](../index.md)에서 S&P 500 기관 포지셔닝(Asset Manager 숏 vs Leveraged Funds)을 **매주 자동 갱신**되는 값·차트로 바로 확인할 수 있습니다
