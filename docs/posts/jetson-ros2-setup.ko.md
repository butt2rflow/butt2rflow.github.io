...
> **현장 노트 · '로봇의 뇌를 엣지에' 2부작 중 1부.** 이번 편은 아무것도 안 깔린 Jetson 보드를 **ROS 2가 도는 상태**까지 세운 기록입니다. 2부는 그 위에 **Isaac ROS**를 올려 GPU 가속 인식을 붙입니다.
...
그리고 솔직한 고백 하나. **지금 실제로 도는 제어 루프는 순수 파이썬**이에요. 카메라에서 프레임을 받아 모델을 돌리고 로봇에 명령을 보내는, 딱 그 수준이죠. ROS 2는 아직 "있으면 좋은 선택적 인프라"에 가깝습니다. 그럼 왜 굳이 ROS 2를 깔았을까요?
