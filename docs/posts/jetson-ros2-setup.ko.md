...
lang: ko
description: "로봇 비전 파이프라인은 결국 셀에 붙은 작은 컴퓨터 위에서 돌아야 한다. Jetson Orin NX에 JetPack과 ROS 2를 올리기까지 — 실제로 부딪힌 벽들의 현장 노트. 2부작 중 1부(진행형)."
---

# 로봇의 뇌를 엣지에 올리기 (1) — JetPack부터 ROS 2까지, 내가 부딪힌 벽들

> **현장 노트 · '로봇의 뇌를 엣지에' 2부작 중 1부.** 이번 편은 아무것도 안 깔린 Jetson 보드에서 **ROS 2가 도는 상태**까지 실제로 걸어간 길입니다. 2부(예정)는 그 위에 **Isaac ROS**를 올려 GPU 가속 인식을 붙이는 이야기 — 아직 안 밟은 다음 발걸음이라, 실제로 올린 뒤에 씁니다.
...
- **큰 그림:** 무거운 **학습은 데스크톱 GPU**에서, **추론은 Jetson**에서. ROS 2는 다음 단계인 **Isaac ROS**로 가는 길목이고, 그건 2부에서.
...
- **학습은 데스크톱 GPU에서.** 시뮬레이션과 모델 학습처럼 무거운 일은 RTX급 데스크톱 GPU가 맡아요. (이건 젯슨에서 못 합니다 — 뒤에 나올 함정이기도 하고요.)
...
## 다음 편 — Isaac ROS, 그런데 갈림길이 있다

ROS 2까지 왔으니 다음은 **Isaac ROS**입니다. NVIDIA가 만든, GPU로 가속되는 ROS 2 인식 패키지 모음이에요. 로봇 비전 시리즈에서 본 그 파이프라인 — 스테레오 깊이, 세그멘테이션, 6-DoF 포즈 — 을 젯슨 위에서 zero-copy로 빠르게 돌리는 실장판이라고 보면 됩니다.

그런데 오르기 전에 지도를 봐야 해요. 최근 Isaac ROS는 **두 갈래로 갈라졌습니다** — 옛 JetPack 6/ROS 2 Humble 계열과, 최신 JetPack 7/ROS 2 Jazzy 계열. 제가 선 **JetPack 6.2.2**는 딱 그 갈림길 근처예요. (앞서 '안정·지원'을 이유로 6을 택한 논리대로면 Humble 쪽이 자연스럽지만, 그게 정말 매끄럽게 붙는지는 해봐야 알죠.) 어느 쪽으로 갈지, 그게 기존 ROS 2 버전에 어떤 영향을 주는지 — 그리고 실제로 컨테이너를 띄우고 첫 인식을 돌리기까지는, **진짜로 올린 다음** 2부에서 정직하게 쓰겠습니다.

지도를 다 그렸으니, 이제 오를 차례입니다.
