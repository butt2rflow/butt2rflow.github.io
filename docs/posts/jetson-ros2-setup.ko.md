...
> **현장 노트 · '로봇의 뇌를 엣지에' 2부작 중 1부.** 이번 편은 아무것도 안 깔린 Jetson 보드를 **ROS 2가 동작하는 상태**까지 세운 기록입니다. 2부는 그 위에 **Isaac ROS**를 올려 GPU 가속 인식을 붙입니다.

먼저 이 글이 어떻게 쓰였는지부터 짚을게요. 여기 나오는 셋업의 상당 부분을 저는 직접 키보드로 치지 않았습니다. 시작할 때 Jetson에 **SSH를 열어 에이전트형 AI(2세대·Claude)가 엣지 컴퓨터에 직접 들어오게** 해뒀고, 그 뒤로는 AI가 제 손보다 빠르게 명령을 치고 로그를 읽고 막힌 데를 풀었어요. 혼자였다면 며칠 헤맸을 벽들을, AI를 옆에 두고 함께 넘었어요. 어떻게 붙였는지는 잠시 뒤에 보고, 먼저 무엇을 왜 만드는지부터 볼게요.
...
이 글은 그 "어딘가"를 세우는 이야기입니다. **Jetson Orin NX**라는 작은 엣지 컴퓨터에, 로봇의 뇌가 될 스택을 바닥부터 올린 기록이에요. 박스를 뜯고 나서 ROS 2가 동작하기까지, 저를 가로막은 벽들까지 정직하게 적었습니다.
...
- **JetPack**(=Jetson 전용 OS+드라이버+CUDA 묶음) **6**으로 플래시 → Ubuntu 22.04·CUDA 12.6. (일부러 최신 7이 아니라 6을 골랐어요. 더 안정적이고 생태계 지원이 두껍습니다.) 여기서 첫 벽들이 나옵니다.
- **ROS 2 Humble**을 apt로 설치. 로봇 소프트웨어의 공통어예요. 여기까진 비교적 순탄합니다.
- **제일 큰 벽:** `pip install torch`가 엉뚱한 CUDA 버전을 끌어와 안 동작해요 → Jetson 전용 인덱스로 받아야 해결됩니다. 데스크톱 상식이 배신하는 지점.
- **큰 그림:** 무거운 **학습은 데스크톱·클라우드 GPU**에서, **추론은 Jetson**에서. ROS 2는 다음 단계인 **Isaac ROS**로 가는 길목이고, 그건 2부에서.
- **어떻게 빨랐나:** 시작할 때 Jetson에 **SSH를 열어 에이전트형 AI(2세대·Claude)가 엣지 컴퓨터에 직접 접근**하게 해두고 출발했어요. 벽에 막힐 때마다 AI가 로그를 읽어가며 풀어서, 혼자 할 때보다 훨씬 빨랐습니다.
...
3. Jetson의 IP를 확인하고(고정 IP 권장), 데스크톱에서 **SSH 키 기반 로그인**을 걸어 비밀번호 없이 들어가게 합니다(`ssh-keygen` → 공개키를 엣지 컴퓨터로 복사).
4. 그다음 그 **SSH 접속을 에이전트형 AI(Claude)에게 넘깁니다.** 이제 AI가 엣지 컴퓨터에서 직접 명령을 치고, 출력과 로그를 읽고, 막히면 스스로 고쳐요. (원격에서 붙어야 하면 Tailscale 같은 걸로 어디서든 같은 주소가 되게 해두면 편합니다.)

이 **한 번의 셋업이 나머지 전부의 속도를 바꿨습니다.** 앞으로 나올 벽들(CUDA 불일치, 커널 붕괴, PATH 문제)도 상당수는 제가 아니라 옆에 앉은 AI가 로그를 읽어가며 풀었어요.

![SSH로 에이전트형 AI를 엣지 컴퓨터에 들여보내기](../assets/diagrams/jetson-ssh-agent.svg)

여기에 재미있는 포개짐이 있어요. 로봇 비전 시리즈에서 다룬 세대 이야기, 그러니까 1세대 생성형(글·그림), 2세대 에이전트형(스스로 판단하고 도구를 쓰는), 3세대 피지컬(로봇 속으로 들어간 AI)을 떠올려 보면, 저는 **2세대 에이전트형 AI를 써서, 3세대 피지컬 AI가 착륙할 엣지 박스를 세웠습니다.** 2세대가 3세대의 활주로를 깔아준 거죠. 그러니 이게 전문가만의 영역이라는 얘기는 아니에요. AI를 옆에 두니 제 지식 너머도 넘을 수 있었습니다.

---

## 왜 엣지인가, 왜 Jetson인가

로봇이 부품을 보고 집는 루프, 그러니까 보고 판단하고 움직이고 힘을 느껴 보정하는 그 흐름은 **밀리초 단위**로 돌아야 합니다. 이걸 클라우드로 왕복시키면? 지연이 치명적이고, 인터넷이 끊기면 로봇이 멈추고, 공장 보안상 외부 연결 자체가 금지인 경우도 많아요. 그래서 인식과 판단은 **셀에 붙은 컴퓨터에서 로컬로** 돌아야 합니다.
...
엣지 컴퓨터는 원래 구형 JetPack(5.x)으로 왔어요. 그래서 더 최신 CUDA·Ubuntu·ROS 2를 쓰려고 **JetPack 6.2.2**(L4T R36.5, Ubuntu 22.04, CUDA 12.6, Python 3.10)로 재플래시했습니다. 플래시는 우분투 호스트 PC에서 **SDK Manager**로, 엣지 컴퓨터를 리커버리 모드에 넣고 NVMe에 굽는 방식이에요.

다만 여기서 한 가지는 **의도적인 선택**이었어요. 가장 최신인 **JetPack 7이 아니라 6에 멈춘 것**입니다. 엣지에서는 '가장 새것'이 답이 아니거든요. JetPack 6은 이미 안정화가 충분히 됐고, 드라이버·미리 빌드된 ML 패키지(뒤에 나올 Jetson용 PyTorch)·Isaac ROS·쌓인 커뮤니티 답변까지 **생태계 지원이 두껍습니다.** 최신 7은 앞서가는 만큼 아직 지원이 얇고요. '새것'보다 **'검증되고 받쳐주는 것'**을 택한 겁니다. 현장에 들어갈 물건일수록 이 판단이 중요하니까요.

여기서 저를 가로막은 벽들, 전부 실제로 걸린 것들입니다.
...
교훈 하나. JetPack은 "우분투 + 약간"이 아니라 **"손대면 깨지기 쉬운, 버전이 꽉 맞물린 우분투"**입니다. 데스크톱 리눅스 습관을 그대로 들고 오면 반드시 한 번은 크게 데입니다.
...
인식 모델을 실행하려면 **PyTorch**(딥러닝 프레임워크)를 GPU로 띄워야 합니다. 그래서 아무 생각 없이 `pip install torch`를 쳤죠. 그게 며칠을 잡아먹은 실수였어요.
...
- **학습은 데스크톱(또는 클라우드) GPU에서.** 시뮬레이션과 모델 학습처럼 무거운 일은 RTX급 데스크톱 GPU가 맡아요. 없으면 **클라우드 GPU 렌털**로 하고요. (어느 쪽이든 젯슨에선 못 합니다. 이것도 뒤에 나올 함정이고요.)
- **추론은 Jetson에서.** 학습으로 나온 모델·정책을 Jetson으로 내려서, 셀에서 실시간 인식→행동만 실행합니다.

그리고 솔직한 고백 하나. **지금 실제로 동작하는 제어 루프는 순수 파이썬**이에요. 카메라에서 프레임을 받아 모델을 실행하고 로봇에 명령을 보내는, 딱 그 수준이죠. ROS 2는 아직 "있으면 좋은 선택적 인프라"에 가깝습니다. 그럼 왜 굳이 ROS 2를 깔았을까요?
...
ROS 2까지 왔으니 다음은 **Isaac ROS**입니다. NVIDIA가 만든, GPU로 가속되는 ROS 2 인식 패키지 모음이에요. 로봇 비전 시리즈에서 본 그 파이프라인, 즉 스테레오 깊이·세그멘테이션·6-DoF 포즈를 젯슨 위에서 zero-copy로 빠르게 돌리는 실장판입니다.

미리 짚어둘 갈림길이 하나 있어요. 최근 Isaac ROS는 **두 갈래**로 나뉩니다. 옛 JetPack 6/ROS 2 Humble 계열과, 최신 JetPack 7/ROS 2 Jazzy 계열이죠. 제가 선 **JetPack 6.2.2**에선, 앞서 '안정·지원'을 이유로 6을 택한 그 논리 그대로 **Humble 쪽**이 자연스러운 선택이죠. 2부에서 이 위에 인식 스택을 올립니다.
...
- *NVMe SSD* — 고속 저장장치. Jetson에서 인식 스택을 실행하려면 사실상 필수
