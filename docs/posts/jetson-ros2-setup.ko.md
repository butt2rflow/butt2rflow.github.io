...
이 글은 그 "어딘가"를 세우는 이야기입니다. **Jetson Orin NX**라는 작은 엣지 컴퓨터에, 로봇의 뇌가 될 스택을 바닥부터 올린 기록이에요. 박스를 뜯고 나서 ROS 2가 돌기까지, 저를 가로막은 벽들까지 정직하게 적었습니다.
...
그리고 솔직한 고백 하나. **지금 실제로 동작하는 제어 루프는 순수 파이썬**이에요. 카메라에서 프레임을 받아 모델을 돌리고 로봇에 명령을 보내는, 딱 그 수준이죠. ROS 2는 아직 "있으면 좋은 선택적 인프라"에 가깝습니다. 그럼 왜 굳이 ROS 2를 깔았을까요?
