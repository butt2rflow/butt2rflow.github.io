...
이 시리즈에서 다루는 세 가지 개념 — 리밸런싱, 기하 평균(복리 수익률을 반영하는 평균), 캘리의 기준(Kelly's Criterion — 최적 투자 비중을 계산하는 공식) — 은 모두 **정보가 아닌 수학에 기반한 전략**입니다. AI가 아무리 발전해도 변동성은 사라지지 않고, 복리의 수학도 바뀌지 않습니다.
...
하지만 섀넌은 강연에서 이렇게 **"정확하게 예측"하는 것이 불가능한 랜덤 워크(Random walk — 주가가 동전 던지기처럼 무작위로 움직인다는 모델)에서 돈을 버는 방법**을 제시합니다.
...
> ⚠️ **이 6%는 *이론 한정* 숫자입니다.** 섀넌의 ×2/×½ 모형(매번 두 배 또는 반토막) 위에서만 성립하는 값으로, 실제 주식시장은 이렇게 극단적으로 움직이지 않아요. 실제 시장에서 리밸런싱 프리미엄은 자산 변동성에 따라 **연 0.1~2% 정도**가 일반적입니다 (뒤의 "Lost Decade 60/40" 예시에서 +1~2%/연 확인 가능). 핵심은 *원리*이지 숫자가 아닙니다.
...
더 정확히는, 이 *현금이라는 연료*에는 두 종류가 섞여 있습니다 — 폭락 cascade에서 *살아남기 위한* **방어 자본**과, 위기 한복판에서 *시간 우위를 행사하기 위한* **공격 자본**. 이 둘을 분리해야 비로소 "변동성을 연료로"가 완성됩니다 (실전 분리법은 [현금 비중 글](../posts/cash-allocation.md) Section 4 심화 박스).
...
!!! tip "맛보기로 먼저 — vol drag와 현금 비중"
    원칙편 Part 2·3의 산술 vs 기하 평균 핵심 직관과 "그래서 현금은 얼마나?"의 정량적 답은 무료 글 [**변동성 기반 현금 비중 — Kelly 공식과 위험 신호의 결합**](../posts/cash-allocation.md) 1번 섹션(vol drag)에서 미리 만나볼 수 있습니다.
...
- **4편 우위 없는 게임, 우위 있는 시장** — 시장에는 어떤 구조적 우위가 남아 있는가
